...
| `grounding_sources_N_{prov}_round_{r}` | DeepResearchOrchestrator | DeepResearchOrchestrator (merge) | list[dict] (grounding chunk URIs + titles, Google only) |
| `grounding_supports_N_{prov}_round_{r}` | DeepResearchOrchestrator | -- | list[dict] (grounding supports, Google only) |
| `grounding_queries_N_{prov}_round_{r}` | DeepResearchOrchestrator | -- | list[str] (web search queries, Google only) |
