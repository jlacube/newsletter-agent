...
### T04-01 Spec Compliance Checklist
- [x] File exists and exports `get_gmail_credentials() -> Credentials` - verified in gmail_auth.py
- [x] Reads `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` from env vars - verified in gmail_auth.py
- [x] Returns `Credentials` object with refresh token and Gmail API scopes - verified in gmail_auth.py
- [x] Refreshes expired access token automatically - verified in gmail_auth.py
- [x] Missing env var raises descriptive `GmailAuthError` - verified in gmail_auth.py
- [x] Invalid/revoked refresh token raises `GmailAuthError` with re-run instruction - verified in gmail_auth.py
- [x] No credentials logged or written to files - verified in gmail_auth.py
- [x] Gmail scope is `gmail.send` only - verified in gmail_auth.py

### T04-02 Spec Compliance Checklist
- [x] File exists and exports `send_newsletter_email(html_content, recipient_email, subject) -> dict` - verified in gmail_send.py
- [x] Constructs MIME multipart/alternative with HTML and plain-text parts - verified in gmail_send.py
- [x] Plain-text part generated by stripping HTML tags - verified in gmail_send.py (`_strip_html`)
- [x] Email From/To/Subject set correctly - verified in gmail_send.py
- [x] On success returns `{"status": "sent", "message_id": str}` - verified in gmail_send.py
- [x] On failure returns error dict without raising - verified in gmail_send.py
- [x] Logs send attempt at INFO, errors at ERROR - verified in gmail_send.py

### T04-03 Spec Compliance Checklist
- [x] `save_newsletter_html(html_content, output_dir, newsletter_date) -> str` exists - verified in file_output.py
- [x] Saves to `{output_dir}/{date}-newsletter.html` - verified in file_output.py
- [x] Output directory auto-created with `mkdir(parents=True, exist_ok=True)` - verified in file_output.py
- [x] Returns absolute path of saved file - verified in file_output.py
- [x] Existing file is overwritten (idempotent) - verified in file_output.py
- [x] File written with UTF-8 encoding - verified in file_output.py
- [x] Logs save path at INFO level - verified in file_output.py

### T04-04 Spec Compliance Checklist
- [x] Delivery agent exists as BaseAgent subclass - verified in delivery.py
- [x] Reads `newsletter_html` from state - verified in delivery.py
- [x] Reads `newsletter_metadata` for date and title - verified in delivery.py
- [x] Reads `config_dry_run` and `config_output_dir` from state - verified in delivery.py
- [x] dry_run=false + valid creds: sends email - verified in delivery.py
- [x] dry_run=true: saves HTML to disk - verified in delivery.py
- [x] Email failure: logs error, saves fallback - verified in delivery.py
- [x] Email subject format `{title} - {date}` - verified in delivery.py
- [x] Logs delivery status at INFO, errors at ERROR - verified in delivery.py

### T04-05 Spec Compliance Checklist
- [x] File `setup_gmail_oauth.py` exists at project root - verified
- [x] Opens browser for Google OAuth2 consent - verified in setup_gmail_oauth.py
- [x] Requests `gmail.send` scope with offline access - verified in setup_gmail_oauth.py
- [x] Prints refresh token and instructs user to add to `.env` - verified in setup_gmail_oauth.py
- [x] Accepts `--client-secrets-file` argument - verified in setup_gmail_oauth.py
- [x] Validates client secrets file exists - verified in setup_gmail_oauth.py
- [x] Handles user cancellation gracefully - verified in setup_gmail_oauth.py

### T04-06 Spec Compliance Checklist
- [x] Delivery agent is the final child in root pipeline - verified in agent.py (OutputPhase)
- [x] dry_run=true produces saved HTML without email - verified in delivery.py
- [x] dry_run=false with valid creds sends email - verified in delivery.py
- [x] dry_run=false with invalid creds saves fallback - verified in delivery.py
- [x] End-to-end chain: config -> research -> synthesis -> format -> deliver - verified in agent.py

### T04-07 Spec Compliance Checklist
- [x] Test file exists with at least 8 test cases (has 10) - verified in test_gmail_auth.py
- [x] Tests cover: valid creds, missing vars, whitespace, revoked token, general refresh error - verified
- [x] All tests mock env vars and Google auth library - verified
- [x] Tests verify correct Gmail scope - verified
- [x] Tests verify `GmailAuthError` raised with descriptive messages - verified

### T04-08 Spec Compliance Checklist
- [x] Test file exists with at least 10 test cases (has 11) - verified in test_gmail_send.py
- [x] All tests mock Gmail API client and credentials - verified
- [x] Tests verify MIME multipart/alternative structure - verified
- [x] Tests verify base64url encoding - verified

### T04-09 Spec Compliance Checklist
- [x] Test file exists with at least 6 test cases (has 6) - verified in test_file_output.py
- [x] Tests cover: existing dir, auto-create dir, filename format, content match, overwrite, UTF-8 - verified
- [x] Tests use `tmp_path` fixture - verified
- [x] Tests verify returned path is absolute - verified

### T04-10 Spec Compliance Checklist
- [x] Test file exists with at least 8 test cases (has 8) - verified in test_delivery_agent.py
- [x] Tests cover: send, dry-run, failure fallback, missing HTML, empty HTML, subject format - verified
- [x] Tests mock both Gmail send and file output functions - verified

### T04-11 Spec Compliance Checklist
- [x] Test file exists with at least 4 BDD scenarios (has 4) - verified in test_email_delivery.py
- [x] Scenario: Successful email send - verified
- [x] Scenario: Dry run mode - verified
- [x] Scenario: Email failure with fallback - verified
- [x] Scenario: Expired token with valid refresh - verified
- [x] Tests use mocked Gmail API - verified
...
- [x] **FB-01**: Add Spec Compliance Checklist (Step 2b) for every task T04-01 through T04-11 in the WP file. Each task must have a checklist mapping acceptance criteria to verified status. **RESOLVED**: Checklist added above Activity Log section.
- [x] **FB-02**: Add at least 1 more unit test to `tests/unit/test_delivery_agent.py` to meet the T04-10 requirement of 8+ tests. The missing scenario is "missing newsletter_html key in state" (key absent, not empty string). Verify the agent handles it gracefully. **RESOLVED**: test_missing_newsletter_html_key_uses_empty_string added (8 tests total).
