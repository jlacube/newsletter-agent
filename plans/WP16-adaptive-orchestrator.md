...
#### Spec Compliance Checklist (T16-01)
- [x] FR-ADR-004: max_searches field added to DeepResearchOrchestrator (Pydantic field, default 3)
- [x] FR-ADR-041: _MIN_URLS_THRESHOLD = 15 constant removed; replaced by saturation-based exit
- [x] FR-ADR-061: max_searches param accepted by constructor
- [x] Section 8.1: Constructor contract maintained; new fields have defaults; backward compat preserved
- [x] min_rounds: int = 2 field added for saturation override (FR-ADR-044)
- [x] Import of get_query_expansion_instruction removed; get_planning_instruction and get_analysis_instruction imported
...
#### Spec Compliance Checklist (T16-02)
- [x] FR-ADR-010: PlanningAgent created as LlmAgent named AdaptivePlanner_{idx}_{prov}
- [x] FR-ADR-011: output_key is adaptive_plan_{idx}_{provider}
- [x] FR-ADR-012: Instruction generated by get_planning_instruction(query, topic_name)
- [x] FR-ADR-013: Fallback on invalid JSON: uses original query + default aspects, logs warning
- [x] FR-ADR-014: key_aspects padded to 3 minimum, truncated to 5 maximum
- [x] Section 7.5 PlanningOutput: JSON parsed for initial_search_query, key_aspects, query_intent, search_rationale
- [x] Section 8.1 PlanningAgent contract: No tools, uses self.model
...
#### Spec Compliance Checklist (T16-03)
- [x] FR-ADR-030: AnalysisAgent created as LlmAgent named AdaptiveAnalyzer_{idx}_{prov}_r{round_idx}
- [x] FR-ADR-031: output_key is adaptive_analysis_{idx}_{provider}
- [x] FR-ADR-032: All 8 parameters passed to get_analysis_instruction
- [x] FR-ADR-033: Fallback on invalid JSON: suffix-based query, saturated=false, logs warning
- [x] FR-ADR-034: knowledge_gaps truncated to 5 items max
- [x] FR-ADR-035: When saturated=false and next_query missing, fallback suffix applied
- [x] Section 7.5 AnalysisOutput: JSON parsed for findings_summary, knowledge_gaps, coverage_assessment, saturated, next_query, next_query_rationale
...
#### Spec Compliance Checklist (T16-04)
- [x] FR-ADR-001: Plan-Search-Analyze-Decide loop replaces fan-out query expansion
- [x] FR-ADR-002: PlanningAgent invoked first for max_rounds > 1
- [x] FR-ADR-003: Each round uses query from AnalysisAgent (or planning for round 0)
- [x] FR-ADR-004: Loop bounded by max_rounds and max_searches
- [x] FR-ADR-005: Analysis invoked after each search (except single-round)
- [x] FR-ADR-006: Exit criteria checked after each analysis
- [x] FR-ADR-054: deep_queries_{idx}_{provider} state key no longer written
- [x] Section 6 Flow B: max_rounds==1 skips planning and analysis
- [x] _expand_queries and _parse_variants methods deleted
- [x] Section 10.5: Planning success, round search, round analysis, and completion logger.info calls present
...
#### Spec Compliance Checklist (T16-05)
- [x] FR-ADR-040: Saturation exit when saturated=true AND round_count >= min_rounds
- [x] FR-ADR-041: No URL threshold; replaced by LLM-driven saturation
- [x] FR-ADR-042: full_coverage exit when knowledge_gaps is empty
- [x] FR-ADR-043: max_rounds_reached exit via for-else (logged at WARNING)
- [x] FR-ADR-044: min_rounds safety - saturation overridden when round_count < min_rounds
- [x] FR-ADR-064: search_budget_exhausted exit when searches_done >= max_searches
- [x] Section 10.5: Saturation exit at INFO, max_rounds at WARNING, budget at INFO
...
#### Spec Compliance Checklist (T16-06)
- [x] FR-ADR-050: adaptive_context persisted to state[adaptive_context_{idx}_{prov}] during loop
- [x] FR-ADR-051: Each round appends round_idx, query, findings_summary, knowledge_gaps, urls_found, coverage_assessment, saturated
- [x] FR-ADR-052: prior_rounds_summary formatted from adaptive_context rounds for AnalysisAgent
- [x] FR-ADR-053: Intermediate keys cleaned up after merge
- [x] FR-ADR-055: adaptive_reasoning_chain_{idx}_{prov} preserved (not cleaned up)
- [x] Section 7.2: AdaptiveContext initialized with plan (query_intent + key_aspects)
- [x] Section 7.5: AdaptiveContext structure matches spec contract
...
#### Spec Compliance Checklist (T16-07)
- [x] FR-ADR-003 (SC-ADR-003): Duplicate query detection using a set of used queries
- [x] Section 4.1 Edge Cases: Suffix from _FALLBACK_SUFFIXES appended on duplicate
- [x] Section 10.5: Duplicate query logged at WARNING with correct format
...
#### Spec Compliance Checklist (T16-08)
- [x] FR-ADR-053: _cleanup_state deletes adaptive_plan_, adaptive_analysis_, adaptive_context_, deep_research_latest_, round keys
- [x] FR-ADR-054: deep_queries_ and deep_query_current_ keys no longer exist (removed in T16-04)
- [x] FR-ADR-055: _cleanup_state does NOT delete adaptive_reasoning_chain_ or research_{idx}_{prov}
- [x] Section 7.2/7.3: _merge_rounds concatenates SUMMARY sections, deduplicates SOURCES URLs
- [x] Empty round results handled gracefully in merge
...
#### Spec Compliance Checklist (T16-09)
- [x] FR-ADR-061: max_searches=config.settings.max_searches_per_topic passed to both Google and Perplexity constructors
- [x] FR-ADR-064: min_rounds=config.settings.min_research_rounds passed to both constructors
- [x] Section 8.1: Defaults flow correctly when config omits new fields
- [x] Standard-mode topic construction is unaffected
...
#### Spec Compliance Checklist (T16-10)
- [x] Section 11.1 PlanningAgent: Valid JSON, fallback on invalid JSON, fallback on missing fields, key_aspects padding/truncation, state key verification
- [x] Section 11.1 AnalysisAgent: Valid JSON, fallback on invalid JSON, saturated+null next_query, fallback on missing next_query, gaps truncation, state key
- [x] Section 11.1 Loop: Planning invoked for max_rounds>1, skipped for max_rounds==1, round 0 uses planning query, subsequent rounds use analysis next_query
- [x] Section 11.1 Exit Criteria: saturation+min_rounds, saturation override, empty gaps, max_rounds, search budget, duplicate query
- [x] Section 11.1 State/Merge: Merged output format, intermediate keys cleaned, reasoning chain persisted, URL dedup, empty round, single-round mode
- [x] All tests use mocked LlmAgent invocations (no real API calls)
- [x] Minimum 25 test cases: 46 tests created, exceeding requirement
...
#### Spec Compliance Checklist (T16-11)
- [x] FR-ADR-003: TestParseVariants tests removed (method deleted per spec)
- [x] FR-ADR-041: TestOrchestratorInstantiation updated with max_searches and min_rounds params
- [x] FR-ADR-054: TestCleanupState updated for new key patterns (no deep_queries_ or deep_query_current_)
- [x] SC-ADR-005: URL extraction tests remain unchanged (_extract_urls method preserved)
- [x] Factory tests verify max_searches and min_rounds are passed to orchestrator
...
- [x] **FB-01**: Add per-task Spec Compliance Checklist (Step 2b) sections for each task T16-01 through T16-11 in this WP file. Each task needs a checklist of spec refs with checked-off items, matching the format used in WP17 after its remediation.
- [x] **FB-02**: Add `logger.info` after successful planning at ~L88 in `_run_async_impl`: `"[AdaptiveResearch] Topic {name}/{provider}: Plan - intent: '{intent}', aspects: [{aspects}], initial query: '{query}'"`. Spec Section 10.5 requires this.
- [x] **FB-03**: Add `logger.info` after each analysis phase at ~L182 in `_run_async_impl`: `"[AdaptiveResearch] Topic {name}/{provider} round {N}: {findings_summary}. Gaps: [{gaps}]. Saturated: {saturated}"`. Spec Section 10.5 requires this.
- [x] **FB-04**: Add `logger.info` for completion at ~L243 in `_run_async_impl`: `"[AdaptiveResearch] Topic {name}/{provider}: completed {N} rounds, {M} unique URLs, exit reason: {reason}"`. Currently only an Event is yielded; the spec requires a separate logger.info call.
- [x] **FB-05**: Update the round search log at L135-138 to include the query: change from `"round %d: %d new URLs, %d total"` to `"round %d: searched '%s', %d new URLs, %d total"` with `current_query` as the additional parameter. Spec Section 10.5 requires this format.
...
- [x] **FB-07**: (WARN, acknowledged) All 11 tasks committed in a single commit `c9f4ed9`. No retroactive fix required.
