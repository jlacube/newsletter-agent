...
removes broken links from research text before refinement and synthesis.

Spec refs: FR-016 through FR-025, Section 11.3.
"""
...
class TestResearchLinkVerificationIntegration:
    @pytest.mark.asyncio
    ...
    async def test_broken_links_removed_valid_preserved(
        ...
        """Broken URLs removed from research entries; valid ones stay."""
        ...
        # Topic 0: broken link cleaned from research text
        r0 = state["research_0_google"]
        assert "https://broken.example.com/gone" not in r0
        ...
        assert "https://dead.example.com/404" not in r1
        assert "[Cloud Doc](https://cloud.example.com)" in r1

    @pytest.mark.asyncio
    @patch("newsletter_agent.tools.link_verifier_agent.verify_urls")
    async def test_cleaned_state_valid_for_downstream(
        self, mock_verify, synthesis_state_with_mixed_urls
    ):
        ...
        # Broken link URLs should not appear in any research entry
        for key in ["research_0_google", "research_1_google"]:
            text = state[key]
            assert "https://broken.example.com/gone" not in text
            assert "https://dead.example.com/404" not in text
